...
module.exports = function(RED) {
    function CreateDirectoryNode(config) {
        try {          
            ...
            node.on('input', async function(msg) {
                try {
                    ...
                    const createDirPath = msg.payload.path;
                    ...
                    if (!path.isAbsolute(createDirPath)) {
                        const err = `Invalid path: ${createDirPath} must be an absolute path.`
                        node.status({ fill: "red", shape: "dot", text: err});
                        throw new Error(err)
                    }
                    ...
                    const jobResponse = await awxConnector.executeJob("Create Directory", this.hostSshService.host, extraVars, timings);
    ...
    RED.nodes.registerType("create-directory", CreateDirectoryNode);
}
